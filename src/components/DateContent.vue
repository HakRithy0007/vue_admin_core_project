<template>
    <div
        class="flex flex-col sm:flex-row sm:gap-2 md:gap-6 justify-between items-center w-full h-[100px] md:h-[50px] lg:h-[50px]0">

        <!-- Date Picker Dropdown -->
        <div class="left w-full lg:w-[20%] h-full justify-start items-center ">
            <v-menu v-model="menu" :close-on-content-click="false" transition="scale-transition" offset-y>
                <template #activator="{ props }">
                    <v-text-field class="poppins" v-bind="props" v-model="formattedDate" label="Select Date" readonly
        ...
        <div class="right w-full lg:w-[80%] grid grid-cols-3 gap-4 lg:flex lg:justify-end lg:items-center">
            ...
            <v-btn :ripple="false" v-for="btn in buttons" :key="btn.value" type="submit" :class="[
                '!h-[40px] sm:!h-[48px] lg:!w-[150px] !rounded-[4px] flex justify-center items-center gap-2',
                activeButton === btn.value ? 'bg-warning text-white' : ''
            ]" @click="setActive(btn.value)">
                <div class="poppins">{{ btn.label }}</div>
            </v-btn>
        </div>
    </div>
</template>
